<template>
	<ol class="asset-list">
		<li>资产 <van-icon name="arrow-down" @click.self="$emit('toggleShow')" class="pull-down" /> <span class="asset-price" >0.00</span></li>

		<li v-for="item in List.Negative" :key="item.text" v-show="item.show"  :style="{'background':item.bgColor}" class="account-list" @click="$emit('detailPage',item.text,item.bgColor)" >
		...
		<li> <van-icon name="plus " class="icon"  @click="$emit('showAdd')"/> 添加账户</li>
	</ol>
</template>
